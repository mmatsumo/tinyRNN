...
class MillerRatDataset(BaseTwoStepDataset):
    ...
    def __init__(self, data_path=None, behav_data_spec=None, neuro_data_spec=None, verbose=True):
        ...
        # warning: do not change the order of the following lines: m70, m55, m64, m71, m88, will affect the order of the sessions when animal_name='all'
        self.sessions = {
            ...
            'm88': ['2016-07-18','2016-07-19','2016-08-01','2016-08-03','2016-09-02','2016-09-05'], # trials < 4000, not analyzed for tiny RNN
            }
        self.sessions['all'] = [sess for animal in self.sessions for sess in self.sessions[animal]]
        self.subject_remapping = {
            'm55': 0, 'm64': 1, 'm70': 2, 'm71': 3, 'm88': 4,}
    ...
    def _detect_trial_type(self, behav=None):
    ...
    def load_data(self, behav_data_spec, neuro_data_spec=None, verbose=True):
        ...
        if 'animal_name' in behav_data_spec and 'session_name' not in behav_data_spec:
            ...
            behav_data_spec['session_name'] = []
            for animal in animal_name:
                behav_data_spec['session_name'].extend(self.sessions[animal])
        ...
        for sess_name in behav_data_spec['session_name']:
            ...
            # find subject id from session name
            sub_id = -1
            for id in self.sessions:
                if sess_name in self.sessions[id]:
                    sub_id = self.subject_remapping[id] + sub_index_start
                    break
            if sub_id < 0:
                raise ValueError(f'WARNING: session {sess_name} not found in sessions; IGNORED')
            nTrials = dt['nTrials']
            ...
            if len(neuro_data_spec) > 0: # load neural data if specified
                n_events = 4

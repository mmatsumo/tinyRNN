...
class GillanHumanDataset(BaseTwoStepDataset):
    ...
    def load_data(self, behav_data_spec, neuro_data_spec=None, verbose=True):
        ...
        if not os.path.exists(self.data_path / 'Gillandata_behav.pkl'):
...
def _combine_subjects_data(data_path):
    ...
    file_path = (r'D:\OneDrive\Documents\git_repo\cognitive_dynamics\files\Gillandata\Experiment 1\self_report_study1.csv')
    df1 = pd.read_csv(file_path)
    df1 = df1.rename(columns={'Unnamed: 0': 'sub_id', 'subj.x': 'subj'})
    df1['filepath'] = 'Experiment 1/twostep_data_study1'

    file_path = (r'D:\OneDrive\Documents\git_repo\cognitive_dynamics\files\Gillandata\Experiment 2\self_report_study2.csv')
    ...
    for i, row in df.iterrows():
        ...
        print(sub_id, subj)
        task_dict = load_subject_data_csv(os.path.join(r'D:\OneDrive\Documents\git_repo\cognitive_dynamics\files\Gillandata',
                                              row['filepath'],
                                              subj+'.csv'
                                              ))

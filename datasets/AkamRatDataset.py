...
Data from Akam's mice. Although the data is from mice, just call it AkamRatDataset due to a naming mistake at the very early stage.
"""
import numpy as np
from .BaseTwoStepDataset import BaseTwoStepDataset, _segment_long_block
import joblib
...
class AkamRatDataset(BaseTwoStepDataset):
    ...
    Akam's rats.
    Load preprocessed data generated from his Python code. See analysis_code/two_step/Two_step.py in his repo.
    ...
    def __init__(self, data_path=None, behav_data_spec=None, neuro_data_spec=None, verbose=False):
        ...
        if self.task == 'reversal_learning': # reversal learning from the same paper
            # mice index [358 359 360 361 367 368 380 382 383 388]
            self._data_dict_list = joblib.load(data_path / 'two_step_ACC_reversal_learning.pkl')
            self.unique_trial_type = 4
        elif self.task == 'no_transition_reversal':  # the reduced two-step task with no transition reversal  from the same paper
            # mice index [269 270 271 272 273 274 275 277 278 279]
            self._data_dict_list = joblib.load(data_path / 'two_step_ACC_no_transition_reversal.pkl')
            self.unique_trial_type = 8
        elif self.task == 'baseline': # the baseline novel two-step task
            self._data_dict_list = joblib.load(data_path / 'two_step_ACC_baseline_sessions.pkl')
            self.unique_trial_type = 8
        else:
            raise ValueError('Invalid task name.')
        self.subject_IDs = np.unique([s['subject_ID'] for s in self._data_dict_list]) # reindex subject_IDs
    ...
    def data_summary(self):
        """Print a summary of the dataset (working for baseline and no_transition_reversal tasks only).
    ...
    def load_data(self, behav_data_spec, neuro_data_spec=None, verbose=True):
        ...
        for s in loaded_sessions:
            ...
            sub_id = self.subject_remapping[s['subject_ID']] + sub_index_start
            nTrials = s['nTrials']
            ...
            if self.task == 'reversal_learning':
                ...
                trial_trans_rule_segments = _segment_long_block(s['trial_trans_rule'][eff_trials], max_trial_num=max_segment_length)
                behav.setdefault('trial_trans_rule', []).extend(trial_trans_rule_segments)
            ...
            behav.setdefault('task_id', []).extend([task_id] * len(action_segments))
        ...
        if 'augment' in behav_data_spec and behav_data_spec['augment']: # only used in distillation
            self._augment_data()
        self._detect_trial_type()
        print("===loaded all===", self.task)
        print('Total batch size:', self.batch_size)
        print('Total trial num:', self.total_trial_num)

    def _augment_data(self):
        """Augment data by flipping action and stage2. Only augment the data for the students"""
        behav = self.behav
        ...
        for b in range(number_block_before_aug): # flip action
            ...
            behav['task_id'].append(behav['task_id'][b])

"""
Run constrained models on Gillan Human. Testing augmentation level = 2.
"""
import sys
sys.path.append('..')
...
if run_student_rnn:
      ...
      with set_os_path_auto():
            teacher_summary = joblib.load(ANA_SAVE_PATH / 'exp_Gillan' / f'rnn_final_best_summary.pkl')
      teacher_summary = teacher_summary[(teacher_summary['hidden_dim'] == 100)]
      dt = Dataset('GillanHuman', behav_data_spec={'augment': True,
                                                   'augment_level': 2,
                                                   },verbose=False)
      for outer_fold in pd.unique(teacher_summary['outer_fold']):
            ...
            for test_index in test_indexes: # for each subject
                  ...
                  # student RNN with blockwise readout
                  base_config.update({
                        'readout_FC': False,
                    })
                  ...
                  behavior_cv_training_config_combination(base_config, config_ranges, n_jobs=1, verbose_level=1)
                  # behavior_cv_training_job_combination(base_config, config_ranges, resource_dict)
                  base_config.pop('readout_FC', None)

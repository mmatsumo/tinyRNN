...
trainval_percent_list = [100, 90, 80, 70, 60, 50, 40, 30, 20]
...
# these trainval_percent of 18 & 9 is not what we what; we want to measure the final number of trainval trials, not training trials alone
# config_ranges.update({
#       'rnn_type': ['GRU'],
#       'l1_weight': [1e-4, 1e-3, 1e-2, 1e-1],
...
config_ranges.update({
      'rnn_type': ['GRU'],
      'l1_weight': [1e-4, 1e-3, 1e-2, 1e-1],
      'trainval_percent': [5, 10],
      'inner_splits': [3],
})
# behavior_cv_training_job_combination(base_config, config_ranges, {'memory': 5, 'cpu': 1, 'gpu': 1})
...
config_ranges = {  # keys are also used to generate model names
      'cog_type': [
            #'BAS',
            #'RC',
            'MB0', 'MB1',#'MB0s', 'MB0se', 'MB0md', 'MB0m', , 'LS0', 'LS1', 'Q(0)',
            # 'MB0mdnb',
      ],
      'trainval_percent': trainval_percent_list,
}
...
# })
config_ranges.update({
        'trainval_percent': [5,10], # equal to 10% and 5% of the original data with inner_split=2
        'inner_splits': [3],
})
# behavior_cv_training_job_combination(base_config, config_ranges, resource_dict, n_jobs=-1)

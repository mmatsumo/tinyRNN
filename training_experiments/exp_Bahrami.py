"""
Run all models on Bahrami Human, four-armed bandit.
"""
import sys
sys.path.append('..')
...
# student RNN, first-level augmentation
run_student_rnn = False
if run_student_rnn:
      for k in ['outer_splits', 'inner_splits', 'single_inner_fold']:
              base_config.pop(k, None)
      with set_os_path_auto():
            teacher_summary = joblib.load(ANA_SAVE_PATH / 'exp_Bahrami' / f'rnn_final_best_summary.pkl')
      teacher_summary = teacher_summary[(teacher_summary['hidden_dim'] == 20)]
      if 'distill' in teacher_summary.columns:
            teacher_summary = teacher_summary[teacher_summary['distill'] == 'none'] # not student
      dt = Dataset('BahramiHuman', behav_data_spec={'augment': True,},verbose=False)

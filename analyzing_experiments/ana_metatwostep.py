...
def run_itself_scores_exp(exp_folder):
    ...
    model_MB_logits = dt.neuro['MB_policy_logits'] # shape: episode_num, trial_num
    ...
    os.makedirs(ana_exp_path / 'metaRL_MB', exist_ok=True)
    # model_scores: 1000
    # trial_type: 1000
    ...
    joblib.dump({
        'scores': model_MB_logits,
        'internal': model_MB_logits,
        'trial_type': dt.behav['trial_type'],
        'hid_state_lb': np.zeros(0),
        'hid_state_ub': np.zeros(0),
    }, ana_exp_path / 'metaRL_MB' / f'total_scores.pkl')
